...
namespace Michsky.LSS
{
    ...
    public class LSS_LoadingScreen : MonoBehaviour
    {
        ...
        public UnityEvent onLoadingDestroy;
        #endregion
        ...
        void Update()
        {
            if (enableVirtualLoading == false) { ProcessLoading(); }
        ...
        {
            int indexHelper = currentImageIndex;
            currentImageIndex = UniqueListIndex(currentImageIndex, imageList.Count);
            if (currentImageIndex == indexHelper) { currentImageIndex = UniqueListIndex(currentImageIndex, imageList.Count); }
            return imageList[currentImageIndex];
        }

        string GetRandomHint()
        {
            int indexHelper = currentHintIndex;
            currentHintIndex = UniqueListIndex(currentHintIndex, hintList.Count);
            if (currentHintIndex == indexHelper) { currentHintIndex = UniqueListIndex(currentHintIndex, hintList.Count); }
            return hintList[currentHintIndex];
        }

        int UniqueListIndex(int currentIndex, int listCount)
        {
            int newIndex = (currentIndex + Random.Range(0, listCount - 1)) % listCount;
            return newIndex;
        }
